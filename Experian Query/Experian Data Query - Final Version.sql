CREATE TABLE dm.experian_report_facebook_202101_to_202107 AS
...
(
    ...
    (
        ...
        , phone_growth
        , lead_arrival_time
        , to_date(lead_arrival_time, 'yyyy-mm-dd') as lead_arrival_date
        ...
        where lower(source) like '%facebook%'
        and to_date(lead_arrival_time, 'yyyy-mm-dd') BETWEEN (select start_date from vars) and (select end_date from vars)
    )
    where rnum = 1
)
,
...
, case when txn_date is not null and lower(transaction_type) = 'fresh' then 1 else 0 end as txn_fresh
, case when txn_date is not null and lower(transaction_type) = 'fresh' and lower(customer_type) = 'new' then 1 else 0 end as txn_fresh_new
, case when lower(source) like '%google%' then 'Google' else 'Others' end as Channel
