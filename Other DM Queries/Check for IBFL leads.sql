...
case when json_extract_path_text(data, 'utm_source',TRUE) = '' and lower(json_extract_path_text(data, 'unbounce_url',TRUE)) not like '%rupeek web%'  then lead_source 
when lower(lead_source) in ('sms','smsrq','airtel','email') then 'SMS' else         json_extract_path_text(data, 'utm_source',TRUE) end             utmSource,
...
SELECT DISTINCT
        ...
        '' AS loanendtime,
        '' AS otp_verified,
        'Reffereal'::varchar AS utmSource,
        '' AS utmMedium,
...
(
    ...
    max_cta_id
    from (
    select distinct *, row_number() over(partition by unbounce_phone,lead_date order by lead_timestamp, lower(source) asc) as row_numb2,
