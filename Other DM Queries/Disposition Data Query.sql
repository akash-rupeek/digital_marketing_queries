TRUNCATE TABLE dm.dr_leads_disposition;
INSERT INTO dm.dr_leads_disposition  
with temp2 as ( select distinct unbounce_phone 
...
temp3 as (
        ...
        FROM(select distinct unbounce_phone,lead_date as date,  lead(lead_date) over(partition by unbounce_phone  order by date asc) as next_lead_date 
                from dm.dr_mapped_leads
...
dm_leads as (select unbounce_phone as unbounce_phone, lead_timestamp::date as lead_date, lead_timestamp lead_datetime
                                from dm.dr_mapped_leads 
...
FROM (SELECT DISTINCT customer_ph_no
                        ,date_time
                        ,total_attempt
                        ,call_duration_secs
...
JOIN (select A.*, case when B.lead_datetime is null then A.lead_date::datetime else B.lead_datetime end as lead_datetime
...
ON A.unbounce_phone = B.lead_phone 
        AND A.lead_date::date = B.lead_date2::date
;