...
(
	...
	from
	(
		select call_data.*, dm_leads.lead_date
		from call_data
		left join dm_leads
		on call_data.customer_ph_no_decrypted = dm_leads.unbounce_phone_decrypted
	) xx
	where extract(week from call_date) = extract(week from lead_date)
),

call_durations_same_week as
(
	select customer_ph_no_decrypted
...
from
(
	select dm_leads.*, call_durations_same_week.*
	, sch
	, txn
	...
	from dm_leads
	left join call_durations_same_week
	on dm_leads.unbounce_phone_decrypted = call_durations_same_week.customer_ph_no_decrypted

	left join dm_txns
	on dm_leads.unbounce_phone_decrypted = dm_txns.unbounce_phone_decrypted
) xx
