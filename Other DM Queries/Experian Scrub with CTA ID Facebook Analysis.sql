...
conversion_rupeek as 
        (
        select distinct unbounce_phone_decrypted as unbounce_phone, lead_date, sch_date, txn_date, customer_type, final_amount, txn
...
(
    ...
    , cnv.sch_date, cnv.txn_date, cnv.customer_type, cnv.txn as rpk_converted
...
, case when max_cta_id in (0, 1) then 'Gold <10 gms' when max_cta_id in (-1) then 'Not Filled' 
when max_cta_id in (2) then 'Gold >10 gms' else 'Others' end as cta_id
, case when customer_type_flag in ('NTGL Affluent', 'NTGL Non  - Affluent') then 'NTGL' else customer_type_flag end as cust_type
, lead_month
, city_abr
, exp_data_flag
, count(distinct(unbounce_phone)) as leads
, count(distinct(case when rpk_converted = 1 then unbounce_phone else null end)) as rupeek_converters
, count(distinct(case when rpk_converted = 1 or other_txn = 1 or next_loan = 1 then unbounce_phone else null end)) as loan_seekers
, count(distinct(case when rpk_converted = 0 and other_txn = 1 then unbounce_phone else null end)) as non_conv_taken_gl
, count(distinct(case when rpk_loan_flag = 0 
	and ((other_txn = 0 and next_loan = 1) 
		or (other_txn = 1 and next_loan = 1 and other_txn_dt <> next_loan_dt))
	then unbounce_phone else null end)) as non_conv_taken_other_loan
from experian_data
where lower(sem_bau_bucket) like '%facebook%'
and lower(customer_type) = 'new'
