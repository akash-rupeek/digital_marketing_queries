truncate table dm.dr_google_fb_leads;

INSERT INTO  dm.dr_google_fb_leads
;
with vars as 
(
	select
	'2021-06-01'::date as start_date
	, current_date - INTEGER '1' AS end_date
	--, '2021-04-04' as end_date
	, 45 as app_attribution_window
	, 14 as others_attribution_window
),
---- unique aaid and name = application installed' ---
branch_data as
(
	...
	where lower(last_attributed_touch_data_tilde_campaign) in (select distinct lower(campaign_name) from dm.facebook_uac_app_campaigns)
),
...
(
	...
	, case when last_app_lead_date is not null then 1 else 0 end as app_lead
	, case when last_app_lead_date < last_channel_lead_date or (last_channel_lead_date is null and last_app_lead_date is not null) then 1 else 0 end as app_attributed_txn
...
(
	...
	where 
		(
			...
			or (lower(utm_source) like ('%regular%') or lower(utm_source) is null )
		)
...
(
	...
	(
		...
		from 
		(
			select *,row_number() over(partition by unbounce_phone,lead_date order by cta_id asc) as row1 
			from
			(
				select distinct 
				json_extract_path_text(data, 'id',TRUE) sessionId,
	...
	inner join leads_mapped b on a.unbounce_phone_decrypted = phone_decrypted and a.lead_date = app_lead_date --and app_source = 'app'--and app_lead = 1
)
,
...
(
	...
	decrypt_mobile_string (phone) AS  unbounce_phone_decrypted,
...
(
	...
	union 
	select * from forms_data_paid
	union 
	select * from Referral_leads_dm
	union 
