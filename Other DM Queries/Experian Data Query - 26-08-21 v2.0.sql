...
(
    ...
    where
    lead_arrival_time :: date BETWEEN (select start_date from vars) and (select end_date from vars)
)
,
...
(
    select distinct
    right(cast(customerid::decimal as text),10) as customerid,
    ...
    from
    (
        select * from dm.experian_scrub_202101
        union all
        select * from dm.experian_scrub_202102
        union all
        select * from dm.experian_scrub_202103_202104
    )

    where right(cast(customerid::decimal as text),10) in (select distinct mobile_number_decrypted from leads)
)
,
...
(
    ...
    and experian_data.open_date :: date <= t2.closed_date :: date + INTERVAL '3 days'
    and experian_data.open_date :: date >= t2.closed_date :: date
)
,
...
(
    ...
    (
        ...
        and leads.lead_arrival_date > experian_data_renewaL_removed.open_date
        group by 1,2
    ) experian_history
)
,
...
(
    ...
    and b.open_date > a.lead_arrival_date
    and b.open_date <= a.next_30_days
